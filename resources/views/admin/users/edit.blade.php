    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="required" for="email">{{ trans('cruds.user.fields.phone') }}</label>
                ...
                <div class="form-group">
                    <label class="" for="gender_id">{{ trans('cruds.voter.fields.gender') }}</label>
                    <select class="form-control select2 {{ $errors->has('gender') ? 'is-invalid' : '' }}"
                            name="gender_id" id="gender_id">
                        @foreach($genders as $id => $entry)
                            <option value="{{ $id }}" {{ (in_array($id, old('gender_id', [])) || $user->gender_id == $id) ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label class="" for="committee_id">{{ trans('cruds.voter.fields.committee') }}</label>
                    <select class="form-control select2 {{ $errors->has('committee') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($committees as $id => $entry)
                            <option value="{{ $id }}" {{ (in_array($id, old('committee_id', [])) || $user->committee_id == $id) ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
