    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assignments.update", [$assignment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('committee') ? 'is-invalid' : '' }}" name="committee_id" id="committee_id" required>
                    @foreach($committees as $id => $entry)
                        <option value="{{ $id }}" {{ (old('committee_id') ? old('committee_id') : $assignment->committee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('area') ? 'is-invalid' : '' }}" name="area_id" id="area_id" required>
                    @foreach($areas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('area_id') ? old('area_id') : $assignment->area->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('area'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender_id" id="gender_id" required>
                    @foreach($genders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('gender_id') ? old('gender_id') : $assignment->gender->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('from') ? 'is-invalid' : '' }}" type="number" name="from" id="from" value="{{ old('from', $assignment->from) }}" step="1" required>
                @if($errors->has('from'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('to') ? 'is-invalid' : '' }}" type="number" name="to" id="to" value="{{ old('to', $assignment->to) }}" step="1" required>
                @if($errors->has('to'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('letters') ? 'is-invalid' : '' }}" name="letters[]" id="letters" multiple required>
                    @foreach($letters as $id => $letter)
                        <option value="{{ $id }}" {{ (in_array($id, old('letters', [])) || $assignment->letters->contains($id)) ? 'selected' : '' }}>{{ $letter }}</option>
                    @endforeach
                </select>
