...
<body class="sidebar-mini layout-fixed {{ app()->getLocale() == 'ar' ? 'rtl' : '' }}" style="height: auto;">
<div class="wrapper">
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown">
                    <a class="nav-link" data-toggle="dropdown" href="#">
                        {{ config('panel.available_languages')[app()->getLocale()]['code'] }}
                    </a>
                    <div class="dropdown-menu dropdown-menu-right">
                        @foreach(config('panel.available_languages') as $langLocale => $langName)
                            <a class="dropdown-item"
                               href="{{ url()->current() }}?change_language={{ $langLocale }}">{{ $langName['code'] }}
                                ({{ $langName['name'] }})</a>
                        @endforeach
                    </div>
                </li>
    ...
    <footer class="main-footer">
        ...
        <strong> &copy;</strong> {{ trans('global.allRightsReserved') }} - تطوير شركة
        <a href="https://sz4h.com" target="_blank">سبيس زوون</a>
    </footer>
